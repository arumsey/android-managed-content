...
%><c:set var="imageTag"><%= "<img ng-src='{{imagePrefix}}" + xssAPI.encodeForHTMLAttr(imageSrc) +
        "' alt='" + xssAPI.encodeForHTMLAttr(image.getAlt()) + "' title='" +
        xssAPI.encodeForHTMLAttr(image.getTitle()) + "' />" %></c:set><%
%>
<% if (image.hasContent()) { %>
<div class="aem-home-banner">
<% } %>
<c:choose>
    <c:when test="${wcmMode}">
        <c:choose>
            <c:when test="${imageHasAnAction}">
                <a pg-navigation="<%= bannerAction %>" pg-navigation-value="<%= bannerValue %>" x-cq-linkchecker="skip">
                    <% image.draw(out); %>
                </a>
            </c:when>
            <c:otherwise>
